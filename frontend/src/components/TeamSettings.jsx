...
import { Users, Mail, Shield, Download, RefreshCw, AlertCircle } from 'lucide-react';
import workspaceService, { WORKSPACE_ROLE_DISPLAY } from '../services/workspaceService';
...
function TeamSettings() {
  ...
  const handleInvite = async (e) => {
    ...
    try {
      ...
      setInviteMsg(`Invite created. Share token: ${invite.token}`);
      setInviteEmail('');
      fetchAll();
    } catch (err) {
            ...
            <div className="mt-4 space-y-2">
              ...
              {invites.map((inv) => (
                <div key={inv.id} className="text-xs text-gray-600 flex justify-between">
                  <span>{inv.email} ({WORKSPACE_ROLE_DISPLAY[inv.role]})</span>
