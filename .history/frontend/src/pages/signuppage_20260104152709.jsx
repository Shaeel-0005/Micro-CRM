...
const SignupPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FFFCF8] text-gray-800 font-sans flex relative overflow-hidden selection:bg-orange-100 selection:text-orange-600">
      ...
      {/* 2. Main Card with Hover Effect */}
      <motion.div 
        variants={containerVariants}
        initial="hidden"
        animate="visible"
        whileHover={{ y: -5, transition: { duration: 0.2 } }}
        className="w-[320px] bg-white rounded-3xl p-8 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] border border-gray-100 relative z-10"
      >
        ...
        <div className="relative w-48 h-48 mx-auto mb-8">
          <svg viewBox="0 0 100 100" className="w-full h-full transform -rotate-90">
            ...
            <motion.circle 
              cx="50" cy="50" r="40" 
              stroke="#FF7F40" strokeWidth="8" fill="none" 
              strokeLinecap="round"
        ...
        {/* 4. Sliding Checklist Items */}
        <div className="space-y-3">
          {[
            { text: "Automated workflows", color: "bg-green-100", iconColor: "text-green-600" },
            { text: "AI-driven insights", color: "bg-green-100", iconColor: "text-green-600" }
          ].map((item, index) => (
            <motion.div 
              key={index}
            ...
            >
              <motion.div 
                ...
                className={`w-5 h-5 rounded-full ${item.color} flex items-center justify-center ${item.iconColor}`}
              >
      ...
      {/* 5. Decorative Floating Element (Small Orange Star/Dot) */}
      <motion.div 
        animate={{ y: [0, -15, 0], x: [0, 10, 0] }}
        transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
        className="absolute top-20 right-20 w-4 h-4 bg-orange-200 rounded-full opacity-50"
      />
    </div>
