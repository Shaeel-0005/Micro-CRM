...
export default function Dashboard() {
  ...
  const calculateStats = () => {
    const totalRevenue = SAMPLE_CONTACTS.reduce(
      (sum, contact) => sum + contact.value,
      0,
    );
    const activeLeads = SAMPLE_CONTACTS.filter(
      (c) => c.status === "hot" || c.status === "warm",
    ).length;
  ...
  const topOpportunities = SAMPLE_CONTACTS.sort((a, b) => b.value - a.value)
    .slice(0, 3)
    .map((contact) => ({
  ...
  const recentActivity = SAMPLE_CONTACTS.slice(0, 3).map((contact) => ({
    action:
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#FFFCF8] text-gray-800 antialiased selection:bg-orange-100 selection:text-orange-600">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed lg:relative w-64 h-full flex flex-col border-r border-gray-100 bg-white z-40 transition-transform duration-300 ease-in-out ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}
      >
        {/* Logo */}
        <div className="flex h-14 sm:h-16 items-center justify-between px-4 sm:px-6 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <span className="text-base sm:text-lg font-medium tracking-tight text-gray-900">
              LeadFlow
            </span>
          </div>
          <button
...
<nav className="flex-1 overflow-y-auto pt-4 sm:pt-6 px-2 sm:px-3 pb-4 sm:pb-6 space-y-1">
  ...
  >
    {({ isActive }) => (
      <>
        <LayoutDashboard className={`h-5 w-5 sm:h-4 sm:w-4 ${isActive ? 'text-[#FF7F40]' : 'text-gray-400 group-hover:text-gray-900'}`} />
        Overview
      </>
    )}
  </NavLink>
  ...
  >
    {({ isActive }) => (
      <>
        <TrendingUp className={`h-5 w-5 sm:h-4 sm:w-4 ${isActive ? 'text-[#FF7F40]' : 'text-gray-400 group-hover:text-gray-900'}`} />
        Pipeline
      </>
    )}
  </NavLink>
  ...
  >
    {({ isActive }) => (
      <>
        <Users className={`h-5 w-5 sm:h-4 sm:w-4 ${isActive ? 'text-[#FF7F40]' : 'text-gray-400 group-hover:text-gray-900'}`} />
        Contacts
      </>
    )}
  </NavLink>
  ...
  >
    {({ isActive }) => (
      <>
        <BarChart3 className={`h-5 w-5 sm:h-4 sm:w-4 ${isActive ? 'text-[#FF7F40]' : 'text-gray-400 group-hover:text-gray-900'}`} />
        Reports
      </>
    )}
  </NavLink>
  ...
  >
    {({ isActive }) => (
      <>
        <Inbox className={`h-5 w-5 sm:h-4 sm:w-4 ${isActive ? 'text-[#FF7F40]' : 'text-gray-400 group-hover:text-gray-900'}`} />
        Inbox
    ...
    <div className="bg-gray-50 rounded-xl p-3 space-y-2 border border-gray-100">
      ...
      <div className="space-y-1">
        ...
        {dropdowns.sales && (
          <div className="pl-9 space-y-1">
            <NavLink to="/pipeline" className="block px-3 py-1.5 text-xs text-gray-600 hover:text-gray-900 rounded-md hover:bg-white">Pipeline</NavLink>
            <NavLink to="/forecasts" className="block px-3 py-1.5 text-xs text-gray-600 hover:text-gray-900 rounded-md hover:bg-white">Forecasts</NavLink>
            <NavLink to="/analytics" className="block px-3 py-1.5 text-xs text-gray-600 hover:text-gray-900 rounded-md hover:bg-white">Analytics</NavLink>
          </div>
        )}
      </div>
      ...
      <div className="space-y-1">
        ...
        {dropdowns.activities && (
          <div className="pl-9 space-y-1">
            <NavLink to="/tasks" className="block px-3 py-1.5 text-xs text-gray-600 hover:text-gray-900 rounded-md hover:bg-white">Tasks</NavLink>
            <NavLink to="/meetings" className="block px-3 py-1.5 text-xs text-gray-600 hover:text-gray-900 rounded-md hover:bg-white">Meetings</NavLink>
            <NavLink to="/calls" className="block px-3 py-1.5 text-xs text-gray-600 hover:text-gray-900 rounded-md hover:bg-white">Calls</NavLink>
          </div>
        )}
      </div>
      ...
      <div className="space-y-1">
        ...
        {dropdowns.integrations && (
          <div className="pl-9 space-y-1">
            <NavLink to="/email" className="block px-3 py-1.5 text-xs text-gray-600 hover:text-gray-900 rounded-md hover:bg-white">Email</NavLink>
            <NavLink to="/calendar" className="block px-3 py-1.5 text-xs text-gray-600 hover:text-gray-900 rounded-md hover:bg-white">Calendar</NavLink>
            <NavLink to="/apps" className="block px-3 py-1.5 text-xs text-gray-600 hover:text-gray-900 rounded-md hover:bg-white">Apps</NavLink>
          </div>
        )}
      </div>
    </div>
        ...
        <div className="border-t border-gray-100 p-3 sm:p-4">
          <a
            href="#"
            className="group flex items-center gap-3 rounded-lg px-3 py-2.5 sm:py-2 text-sm font-medium text-gray-600 hover:text-gray-900 transition-all"
          >
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-4 lg:p-8">
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
            {statsData.map((stat, idx) => (
              <div
                key={idx}
                className="transition-transform hover:-translate-y-0.5 bg-white border-gray-100 border rounded-xl p-4 sm:p-5 shadow-[0_8px_30px_-8px_rgba(255,127,64,0.15)]"
              >
                ...
                <div className="flex flex-col sm:flex-row gap-1 sm:gap-2 mt-2 sm:mt-3 sm:items-baseline">
                  ...
                  {stat.change && (
                    <span
                      className={`flex items-center gap-0.5 text-[10px] sm:text-xs font-medium rounded py-0.5 px-1.5 w-fit ${stat.isPositive ? "text-emerald-600 bg-emerald-50" : "text-rose-600 bg-rose-50"}`}
                    >
                      {stat.isPositive ? (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-gray-100 px-4 sm:px-6 py-4 gap-2">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1 hidden sm:block">
                      Top value deals in your pipeline
                    </p>
                  </div>
                  <button className="rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs font-medium text-gray-600 hover:bg-gray-50 w-fit">
                    View all
                  </button>
                </div>
                      ...
                      <div className="mt-3 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <div className="h-1.5 w-12 rounded-full bg-gray-100">
                            <div
                              className={`h-1.5 rounded-full ${opp.probability >= 70 ? "bg-emerald-500" : "bg-amber-400"}`}
              ...
              <div className="p-4 sm:p-6 space-y-4">
                {recentActivity.map((activity, idx) => (
                  <div key={idx} className="flex gap-3">
                    <div
                      className={`h-8 w-8 rounded-full ${activity.color === "emerald" ? "bg-emerald-100" : "bg-orange-100"} flex items-center justify-center`}
                    >
                      <div
                        className={`h-2 w-2 rounded-full ${activity.color === "emerald" ? "bg-emerald-500" : "bg-[#FF7F40]"}`}
                      />
                    </div>
