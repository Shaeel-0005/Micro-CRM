...
export default function C_Dash() {
  const [sidebarOpen, setSidebarOpen] = useState(false);

  const toggleSidebar = () => setSidebarOpen(!sidebarOpen);

  const stats = [
    { label: 'Total Revenue', value: '$124,500', change: '+12%', isPositive: true, icon: DollarSign },
    { label: 'Active Leads', value: '42', change: '+4', isPositive: true, icon: Target },
    { label: 'Win Rate', value: '38.2%', change: '-1.2%', isPositive: false, icon: PieChart },
    { label: 'Avg Deal Size', value: '$8,400', subtext: 'per deal', icon: Briefcase },
  ];
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden bg-slate-50/50">
        ...
        <header className="flex h-14 sm:h-16 items-center justify-between border-b border-slate-200 bg-white/80 backdrop-blur-sm px-3 sm:px-4 lg:px-8">
          ...
          <div className="flex items-center gap-2 sm:gap-3 lg:gap-6">
            ...
            <button className="inline-flex items-center gap-2 rounded-lg bg-slate-900 px-3 sm:px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-slate-800 transition-all active:scale-95">
